...
from api.v1.auth import schemas as auth_schemas
...
class OAuth2PasswordBearer(OAuth2):
    ...
    async def __call__(self, request: Request) -> Optional[str]:
        ...
        app_id: str = request.headers.get("x-app-id")

        scheme, access_token = get_authorization_scheme_param(authorization)

        if api_key and app_id:
            return auth_schemas.APIAuth(**{"api_key": api_key, "app_id": app_id})

        elif authorization:
            return auth_schemas.Token(token_type=auth_schemas.TokenType.ACCESS, token=access_token)
        
        elif refresh_token:
            return auth_schemas.Token(token_type=auth_schemas.TokenType.REFRESH, token=refresh_token)
        else:
            raise HTTPException(
